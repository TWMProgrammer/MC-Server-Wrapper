...
impl InstanceManager {
    ...
    pub(crate) fn parse_script_content(&self, content: &str) -> ParsedScriptInfo {
        ...
        let java_line = content
            .lines()
            .map(|l| l.trim())
            .find(|l| (l.starts_with("java") || l.contains(" java ")) && l.contains("-jar"));

        if let Some(line) = java_line {
            // Remove 'java' from the start if it exists
            let cmd = if line.starts_with("java") {
                &line[4..].trim()
            } else if let Some(idx) = line.find(" java ") {
                &line[idx + 6..].trim()
            } else {
                line
            };
            ...
            if let Some(jar_idx) = cmd.find("-jar") {
                let jvm_part = &cmd[..jar_idx].trim();
                let rest = &cmd[jar_idx + 4..].trim();

                // Parse JVM args
                let args: Vec<String> =
                    jvm_part.split_whitespace().map(|s| s.to_string()).collect();
                ...
                if let Some(jar) = rest_parts.next() {
                    info.jar_name = Some(jar.to_string());
                    info.server_args = rest_parts.map(|s| s.to_string()).collect();
                }
            }
        }
...
mod tests {
 ...
 goto :start"#;
        ...
        let info = manager.parse_script_content(content);
