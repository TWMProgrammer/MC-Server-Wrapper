...
import { getLanguageFromExtension } from '../utils/monaco'
import {
  X,
  Save,
  RefreshCw,
...
import { InstalledMod, ModConfig } from '../types'
import { useToast } from '../hooks/useToast'
import { cn } from '../utils'
import { useAppSettings } from '../hooks/useAppSettings'

interface ModConfigModalProps {
  mod: InstalledMod;
  instanceId: string;
  onClose: () => void;
}
...
export function ModConfigModal({ mod, instanceId, onClose }: ModConfigModalProps) {
  ...
  const loadConfigs = async () => {
    setLoading(true)
    try {
      const initialConfigs = await invoke<ModConfig[]>('get_mod_configs', {
        instanceId,
        modName: mod.name
      })
      ...
      for (const config of initialConfigs) {
        const files = await invoke<string[]>('list_mod_config_files', {
          instanceId,
          relPath: config.path
        })
        allFiles.push(...files)
      ...
      if (allFiles.length > 0) {
        setSelectedConfig(allFiles[0])
      }
    } catch (err) {
      console.error('Failed to load mod configs:', err)
                  ...
                  {configs.length === 0 && !loading ? (
                    <div className="px-4 py-8 text-center bg-white/5 rounded-2xl border border-white/5">
                      ...
                      <p className="text-xs text-gray-500 leading-relaxed">
                        ...
                        <span className="opacity-50 text-[10px]">Is the mod configured yet?</span>
                      </p>
                    </div>
                  ) : (
