...
export function PlayerAvatar({ name, uuid, settings, className = "w-12 h-12" }: PlayerAvatarProps) {
  ...
  // Only call hook if we should download
  const { localUrl } = usePlayerHead(shouldDownload ? headIdentifier : null);
  ...
  return (
    <div className={`${className} rounded-xl bg-black/5 dark:bg-white/5 flex items-center justify-center text-gray-400 border border-black/5 dark:border-white/5`}>
