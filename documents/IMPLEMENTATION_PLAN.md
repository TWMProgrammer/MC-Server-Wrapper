...
- [x] **Application Versioning**
  - [x] Implement version synchronization script (`sync-versions.js`) to align `package.json`, `Cargo.toml`, and `tauri.conf.json`.
  - [x] Dynamic UI versioning using Tauri's `getVersion()` API.
  - [x] Replace hardcoded versions in Rust backend (user-agents, logs) with `env!("CARGO_PKG_VERSION")`.
