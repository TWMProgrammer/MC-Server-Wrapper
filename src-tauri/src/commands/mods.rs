...
use mc_server_wrapper_core::mods::{self, InstalledMod, Project, ModProvider, SearchOptions, ModConfig, ModUpdate};
use tauri::State;
use std::sync::Arc;
use uuid::Uuid;
...
pub async fn get_mod_dependencies(
    ...
    let cf_api_key = std::env::var("CURSEFORGE_API_KEY").ok();
    mods::get_mod_dependencies(&project_id, provider, cf_api_key).await.map_err(|e| e.to_string())
}
